...
export function LogsPage({ logs, onRefresh }: { logs: LogItem[]; onRefresh: () => void }) {
  if (logs.length === 0) {
    return <><PageHeader title="运行日志" description="查看当前 Agent 最近的运行事件与错误。" actions={<IconButton icon={RefreshCw} label="刷新日志" onClick={onRefresh} />} /><div className="card"><div className="card-body"><EmptyState icon={FileText} title="暂无日志记录" text="Agent 产生新的运行事件后会显示在这里。" /></div></div></>;
  }
  return (
    <>
      <PageHeader title="运行日志" description="查看当前 Agent 最近的运行事件与错误。" actions={<IconButton icon={RefreshCw} label="刷新日志" onClick={onRefresh} />} />
      <div className="card">
