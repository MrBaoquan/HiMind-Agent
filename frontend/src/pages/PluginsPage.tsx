...
import { Blocks, Check, Download, ExternalLink, FolderOpen, FlaskConical, LockKeyhole, MonitorUp, Play, RefreshCw, Search, ShieldCheck, Unplug } from 'lucide-react';
...
export function PluginsPage({ registry, catalog, capabilities, onRefresh, onInstall, onUninstall, onRollback, onSetEnabled, onOpenDirectory, onRegisterDevelopment, onUnregisterDevelopment, onInvokeDevelopment, onOpenView, onCreateShortcut }: {
  ...
  return (
    <>
      <div className="plugin-detail-header">
        ...
        <div className="actions-row">{item.development ? <button className="btn btn-danger-quiet" onClick={() => onUnregisterDevelopment(item.id)}><Unplug size={15} />移除开发注册</button> : <><button className="btn" disabled={item.governance === 'required' || item.governance === 'managed'} title="核心或组织管理插件不可停用" onClick={() => onSetEnabled(item.id, !item.enabled)}>{item.enabled ? '停用' : '启用'}</button>{item.governance !== 'required' && item.governance !== 'managed' && item.governance !== 'blocked' ? <button className="btn" disabled={!item.rollback_available} title={item.rollback_available ? `切换到 ${item.previous_version}` : '暂无可用上一版本'} onClick={() => onRollback(item.id)}>回滚{item.previous_version ? `到 v${item.previous_version}` : ''}</button> : null}<button className="btn btn-danger-quiet" disabled={item.governance === 'required' || item.governance === 'managed'} title="核心或组织管理插件不可卸载" onClick={() => onUninstall(item.id)}>卸载</button></>}</div>
      </div>
