...
export function SkillsWorkspacePage({ catalog, status, error, marketplace, marketplaceError, availablePlugins, drafts, submissions, busyAction, syncMode, onSetSyncMode, onRefresh, onSyncAll, onSyncSkill, onPlanMarketplace, onInstallMarketplace, onSaveDraft, onCreateRevision, onTestDraft, onConfirmDraft, onSubmitDraft, onRepair, onUninstall, onOpenDirectory }: SkillsWorkspacePageProps) {
    ...
    <div className="skill-page skill-product-page">
      <PageHeader
        ...
        actions={<>
          ...
          {view === 'installed' ? <button className="btn btn-primary" onClick={onSyncAll} disabled={isBusy || !items.length}><RefreshCw className={busyAction === 'sync-all' ? 'spin' : ''} size={16} />{busyAction === 'sync-all' ? '正在同步' : '同步全部'}</button> : null}
          {view === 'installed' ? <button className="btn btn-icon" title="打开默认 Skill 目录" aria-label="打开默认 Skill 目录" onClick={() => status?.target_root && onOpenDirectory(status.target_root)} disabled={!status?.target_root}><FolderOpen size={16} /></button> : null}
          <button className="btn btn-icon" title="刷新状态" aria-label="刷新状态" onClick={onRefresh} disabled={isBusy}><RefreshCw size={16} /></button>
        </>}
      />

      <SkillClientSummary status={status} syncMode={syncMode} />

      {error ? <div className="blocker"><CircleAlert size={18} /><div><strong>Skill 状态读取失败</strong><span>{error}</span></div></div> : null}

      {status?.target_mode === 'preview' && view !== 'marketplace' ? <div className="skill-inline-warning"><CircleAlert size={15} /><span>尚未连接默认 Skill 目录，当前安装仅用于本机预览。</span></div> : null}
	  ...
	  <div className="plugin-toolbar skill-view-toolbar"><div className="plugin-tabs" role="tablist" aria-label="Skill 视图">
	    <button role="tab" aria-selected={view === 'marketplace'} className={view === 'marketplace' ? 'active' : ''} onClick={() => setView('marketplace')}>技能市场 <span>{marketplace.length}</span></button>
	    ...
	    <button role="tab" aria-selected={view === 'mine'} className={view === 'mine' ? 'active' : ''} onClick={() => setView('mine')}>我的创作 <span>{drafts.length}</span></button>
	  </div>{view === 'mine' ? <Pill kind="warn">开发者工作区</Pill> : null}</div>
	  {view === 'installed' ? <div className="skill-sync-settings"><div><strong>同步方式</strong><span>{syncMode === 'symlink' ? '软链接' : '复制文件'}</span></div><div className="segmented-control" role="group" aria-label="Skill 同步方式"><button type="button" className={syncMode === 'copy' ? 'active' : ''} disabled={isBusy} onClick={() => onSetSyncMode('copy')}><Files size={14} />复制文件</button><button type="button" className={syncMode === 'symlink' ? 'active' : ''} disabled={isBusy} onClick={() => onSetSyncMode('symlink')}><Link2 size={14} />软链接</button></div></div> : null}
      ...
      <section className="skill-workspace">
        <aside className="skill-browser">
          <div className="skill-browser-tools">
            ...
            {view === 'marketplace' ? <div className="market-category-block"><div className="market-category-heading"><strong>功能分类</strong><span>按用途查找</span></div><nav className="market-category-nav" aria-label="Skill 功能分类"><button type="button" className={categoryFilter === 'all' ? 'active' : ''} onClick={() => setCategoryFilter('all')}>全部技能<span>{marketplace.length}</span></button>{FUNCTIONAL_CATEGORIES.map(category => <button type="button" key={category.id} className={categoryFilter === category.id ? 'active' : ''} onClick={() => setCategoryFilter(category.id)}>{category.label}<span>{categoryCounts.get(category.id) || 0}</span></button>)}</nav></div> : null}
          </div>
          <div className="skill-browser-list">
			...
			{(view === 'marketplace' ? !visibleMarket.length : view === 'mine' ? !visibleDrafts.length : !filteredItems.length) ? <EmptyState icon={view === 'marketplace' ? Building2 : BookOpen} title={view === 'marketplace' ? '技能库暂无内容' : view === 'mine' ? '还没有创作技能' : '没有匹配的技能'} text={view === 'marketplace' ? '审核通过并正式发布的 AI 技能会出现在这里。' : view === 'mine' ? '创建技能并完成实际测试后，可以提交组织审核。' : '调整搜索内容或筛选条件。'} /> : null}
          </div>
        </aside>

        <main className="skill-detail">
		  {view === 'marketplace' ? (selectedMarket ? <MarketSkillDetail item={selectedMarket} installed={installedById.get(selectedMarket.skill_id)} availablePlugins={availablePlugins} busyAction={busyAction} onPlan={async () => { setPlanLoading(true); setPlanError(''); try { setInstallPlan(await onPlanMarketplace(selectedMarket.skill_id)); } catch (reason) { setPlanError(reason instanceof Error ? reason.message : String(reason)); } finally { setPlanLoading(false); } }} planLoading={planLoading} /> : <EmptyState icon={Sparkles} title="选择一个技能" text="查看用途、所需工具和安全信息。" />) : view === 'mine' ? (selectedDraft ? <DraftDetail item={selectedDraft} availablePlugins={availablePlugins} submission={selectedSubmission} busyAction={busyAction} onCreateRevision={onCreateRevision} onEdit={() => { setEditingDraft(selectedDraft); setDraftEditorOpen(true); }} onTest={onTestDraft} onConfirm={onConfirmDraft} onSubmit={onSubmitDraft} onOpenDirectory={onOpenDirectory} /> : <EmptyState icon={Sparkles} title="创建本地 Skill" text="完成编辑、预检和 Codex 实测后再提交审核。" />) : (selected ? <SkillDetail item={selected} availablePlugins={availablePlugins} catalogPolicy={marketplace.find(item => item.skill_id === selected.record.manifest.id)} busyAction={busyAction} onSync={onSyncSkill} onRepair={onRepair} onUninstall={() => setPendingUninstall(selected)} onOpenDirectory={onOpenDirectory} /> : <EmptyState icon={Sparkles} title="选择一个技能" text="查看用途、安装状态和组织管理信息。" />)}
        </main>
      </section>
	  ...
	  {draftEditorOpen ? <DraftEditor draft={editingDraft} availablePlugins={availablePlugins} onClose={() => setDraftEditorOpen(false)} onSave={async input => { await onSaveDraft(input); setDraftEditorOpen(false); }} /> : null}
...
function MarketSkillListItem({ item, installed, selected, onSelect }: { item: OrganizationSkillCatalogItem; installed?: CodexSkillStatusItem; selected: boolean; onSelect: (id: string) => void }) {
  ...
  const label = update ? '可更新' : installed ? '已安装' : managed ? '部门内置' : item.assignment === 'recommended' ? '组织推荐' : '可安装';
  return <button className={`skill-browser-item ${selected ? 'selected' : ''}`} onClick={() => onSelect(item.skill_id)}><span className={`skill-state-rail ${update ? 'warn' : installed ? 'success' : managed ? 'warn' : 'neutral'}`} /><span className="skill-browser-item-copy"><strong>{item.name}</strong><small>{item.source === 'organization' ? '组织提供' : '公共技能库'} · {item.description || '暂无用途说明'}</small><small className="catalog-item-author">作者：{item.author_name || '马宝全'}</small></span><span className={`skill-state-label ${update ? 'warn' : installed ? 'success' : managed ? 'warn' : 'neutral'}`}>{label}</span></button>;
}
...
function SkillClientSummary({ status, syncMode }: { status: CodexSkillStatusResponse | null; syncMode: SkillSyncSettings['mode'] }) {
  return <section className="skill-client-summary" aria-label="Skill 客户端状态">
    <div className="skill-client-summary-intro"><span className="skill-client-mark">AI</span><div><small>AI 客户端</small><strong>Skill 将同步到已配置客户端</strong></div></div>
    ...
    <div className="skill-client-sync"><div><small>全局同步方式</small><strong>{syncMode === 'symlink' ? '软链接' : '复制文件'}</strong></div><span title="同步方式会应用于全部支持的客户端"><RefreshCw size={15} /></span></div>
  </section>;
}
...
function MarketSkillDetail({ item, installed, availablePlugins, busyAction, onPlan, planLoading }: { item: OrganizationSkillCatalogItem; installed?: CodexSkillStatusItem; availablePlugins: PluginCatalogItem[]; busyAction: string | null; onPlan: () => void; planLoading: boolean }) {
  const update = installed ? compareSemanticVersions(item.version, installed.record.manifest.version) > 0 : false;
  const current = Boolean(installed) && !update;
  const busy = busyAction === `market:${item.skill_id}`;
	const managed = item.assignment === 'required' && item.management !== 'user_managed';
  return <>
    <header className="skill-detail-header"><div className="skill-detail-title"><span className="skill-detail-mark">{item.name.slice(0, 1).toUpperCase()}</span><div><div className="skill-title-line"><h3>{item.name}</h3><Pill kind={managed ? 'warn' : 'success'}><BadgeCheck size={12} />{managed ? '部门内置' : '已审核'}</Pill></div><small className="skill-detail-source">{item.source === 'organization' ? '由组织提供' : '来自公共技能库'} · 作者：{item.author_name || '未知作者'}</small></div></div><div className="skill-detail-actions"><button className="btn btn-primary" disabled={busy || planLoading || current || managed} onClick={onPlan}><Download className={busy || planLoading ? 'spin' : ''} size={15} />{managed ? installed ? '已安装' : '部门内置' : busy ? '正在安装' : planLoading ? '检查所需工具' : update ? '更新' : current ? '已是最新' : '安装'}</button></div></header>
    ...
    <div className="skill-detail-meta"><div><span>可用版本</span><strong>v{item.version}</strong></div><div><span>已安装版本</span><strong>{installed ? `v${installed.record.manifest.version}` : '--'}</strong></div><div><span>发布者</span><strong>{item.author_name || '未知作者'}</strong></div><div><span>操作范围</span><strong>{riskLabel(item.risk_summary)}</strong></div></div>
	<section className="skill-detail-section"><div className="skill-section-title"><div><Building2 size={16} /><strong>所需插件</strong></div><span>{item.plugin_dependencies?.length || 0}</span></div><div className="skill-dependency-list">{(item.plugin_dependencies || []).map(dependency => <div key={dependency.plugin_id}><span className="status-dot success" /><PluginDependencyIdentity pluginId={dependency.plugin_id} plugins={availablePlugins} /><span>{dependency.required ? '必需' : '可选'}</span><strong>{dependency.min_version ? `>= v${dependency.min_version}` : '任意版本'}</strong></div>)}{!item.plugin_dependencies?.length ? <span className="skill-section-empty">无需关联插件</span> : null}</div></section>
    <details className="plugin-technical-panel"><summary>版本与技术信息</summary><div className="skill-release-grid"><div><small>最低 Agent</small><strong>v{item.min_agent_version || '--'}</strong></div><div><small>支持客户端</small><strong>{item.supported_clients.join(', ')}</strong></div><div><small>签名密钥</small><code>{item.signature_key_id}</code></div><div><small>SHA-256</small><code title={item.sha256}>{item.sha256.slice(0, 16)}...</code></div>{item.capability_ids.length ? <div className="skill-technical-wide"><small>Capability</small><code>{item.capability_ids.join(' · ')}</code></div> : null}</div>{item.release_notes ? <p className="skill-release-notes">{item.release_notes}</p> : null}</details>
  </>;
}
...
function DraftEditor({ draft, availablePlugins, onClose, onSave }: { draft: AuthoringSkillDraft | null; availablePlugins: PluginCatalogItem[]; onClose: () => void; onSave: (input: AuthoringSkillDraftInput) => Promise<void> }) {
  ...
  return <div className="skill-dialog-backdrop"><div className="skill-dialog skill-editor-dialog" role="dialog" aria-modal="true"><div className="skill-dialog-head"><strong>{draft ? '编辑 Skill' : '新建 Skill'}</strong><button className="btn btn-icon" onClick={onClose} aria-label="关闭"><X size={16} /></button></div>{saveError ? <div className="skill-dialog-warning"><CircleAlert size={16} />{saveError}</div> : null}<div className="skill-editor-grid">
    <label><span>名称</span><input value={input.name} placeholder="例如：项目交付检查" onChange={event => change('name', event.target.value)} /></label><label><span>作者</span><input value={input.author} disabled /></label><label><span>主功能分类</span><select value={functionalCategoryID(input.categories[0])} onChange={event => changeCategory(event.target.value)}><option value="">请选择分类</option>{FUNCTIONAL_CATEGORIES.map(category => <option key={category.id} value={category.id}>{category.label}</option>)}</select></label><label className="wide"><span>用途说明</span><textarea rows={2} value={input.description} placeholder="说明这个 Skill 适合在什么情况下使用" onChange={event => change('description', event.target.value)} /></label><label className="wide"><span>Skill 指令</span><textarea className="skill-editor-readme" rows={15} value={input.readme} onChange={event => change('readme', event.target.value)} /></label>
...
function PluginDependencyIdentity({ pluginId, plugins }: { pluginId: string; plugins: PluginCatalogItem[] }) {
  ...
  return <span className="skill-dependency-identity"><strong>{plugin?.name || readablePluginID(pluginId)}</strong><code title={pluginId}>{pluginId}</code></span>;
}
...
function SkillDetail({ item, availablePlugins, catalogPolicy, busyAction, onSync, onRepair, onUninstall, onOpenDirectory }: { item: CodexSkillStatusItem; availablePlugins: PluginCatalogItem[]; catalogPolicy?: OrganizationSkillCatalogItem; busyAction: string | null; onSync: (id: string) => void; onRepair: (id: string) => void; onUninstall: () => void; onOpenDirectory: (path: string) => void }) {
  const manifest = item.record.manifest;
  ...
  return <>
    ...
    <section className="skill-detail-section"><div className="skill-section-title"><div><Building2 size={16} /><strong>所需插件</strong></div><span>{manifest.plugin_dependencies?.length || 0}</span></div><div className="skill-dependency-list">
      {(manifest.plugin_dependencies || []).map(dependency => <div key={dependency.plugin_id}><span className="status-dot success" /><PluginDependencyIdentity pluginId={dependency.plugin_id} plugins={availablePlugins} /><span>{dependency.required ? '必需' : '可选'}</span><strong>{dependency.min_version ? `>= v${dependency.min_version}` : '任意版本'}</strong></div>)}
      {!manifest.plugin_dependencies?.length ? <span className="skill-section-empty">无需关联插件</span> : null}
    </div></section>
    <details className="plugin-technical-panel"><summary>同步与技术信息</summary><div className="plugin-technical-grid"><div><span>Skill 标识</span><code>{manifest.id}</code></div><div><span>最近同步</span><strong>{formatSyncedAt(item.last_synced_at)}</strong></div><div><span>托管文件</span><strong>{item.managed_files.length}</strong></div><div><span>Capability</span><strong>{item.readiness.dependencies.length}</strong></div><div className="wide"><span>渲染目录</span><code>{item.rendered_root || '--'}</code></div></div>{item.modified_files.length ? <div className="skill-modified-files">{item.modified_files.map(path => <code key={path}>{path}</code>)}</div> : null}{item.readiness.dependencies.length ? <div className="skill-dependency-list">{item.readiness.dependencies.map(dependency => <div key={dependency.id}><span className={`status-dot ${dependency.state === 'ready' ? 'success' : 'danger'}`} /><code>{dependency.id}</code><span>{dependency.required ? '必需' : '可选'}</span><strong>{dependency.capability_version ? `v${dependency.capability_version}` : dependency.reason || '不可用'}</strong></div>)}</div> : null}<div className="skill-file-summary"><span /><button className="text-action" disabled={!item.rendered} onClick={() => onOpenDirectory(item.rendered_root)}><FolderOpen size={14} />打开目录</button></div></details>
  </>;
}
...
function draftInputFrom(draft: AuthoringSkillDraft | null): AuthoringSkillDraftInput { return draft ? { id: draft.manifest.id, name: draft.manifest.name, author: draft.manifest.author || '马宝全', categories: draft.manifest.categories || [], version: draft.manifest.version, description: draft.manifest.description || '', min_agent_version: draft.manifest.min_agent_version || '0.3.1', supported_clients: draft.manifest.supported_clients || ['codex', 'github-copilot', 'workbuddy'], capabilities: draft.manifest.capabilities || [], plugin_dependencies: draft.manifest.plugin_dependencies || [], risk_summary: draft.manifest.risk_summary || 'read_only', readme: draft.readme, files: draft.files || {} } : { id: `com.himind.skill.custom-${Date.now().toString(36)}`, name: '', author: '马宝全', categories: ['software-engineering'], version: '0.1.0', description: '', min_agent_version: '0.3.1', supported_clients: ['codex', 'github-copilot', 'workbuddy'], capabilities: [{ id: 'system.health', required: true, min_version: '1.0.0', provider: 'agent' }], plugin_dependencies: [], risk_summary: 'read_only', readme: '# 技能名称\n\n## 需要的信息\n\n列出执行前需要向用户确认的信息。\n\n## 执行步骤\n\n1. 检查必要输入与依赖。\n2. 只调用已声明的 Capability。\n3. 汇总结果和需要用户处理的问题。\n\n## 输出要求\n\n说明希望 AI 返回的内容和格式。\n\n## 禁止事项\n\n- 不绕过 Capability Gateway。\n- 不展示账号、Cookie、Token 或其他凭据。\n', files: {} }; }
