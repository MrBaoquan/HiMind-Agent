...
function DraftEditor({ draft, onClose, onSave }: { draft: AuthoringSkillDraft | null; onClose: () => void; onSave: (input: AuthoringSkillDraftInput) => Promise<void> }) {
  ...
  return <div className="skill-dialog-backdrop"><div className="skill-dialog skill-editor-dialog" role="dialog" aria-modal="true"><div className="skill-dialog-head"><strong>{draft ? '编辑 Skill' : '新建 Skill'}</strong><button className="btn btn-icon" onClick={onClose} aria-label="关闭"><X size={16} /></button></div>{saveError ? <div className="skill-dialog-warning"><CircleAlert size={16} />{saveError}</div> : null}<div className="skill-editor-grid">
    <label><span>Skill ID</span><input value={input.id} disabled={Boolean(draft)} onChange={event => change('id', event.target.value)} /></label><label><span>名称</span><input value={input.name} onChange={event => change('name', event.target.value)} /></label><label><span>版本</span><input value={input.version} disabled={Boolean(draft)} onChange={event => change('version', event.target.value)} /></label><label><span>最低 Agent</span><input value={input.min_agent_version} onChange={event => change('min_agent_version', event.target.value)} /></label><label className="wide"><span>用途说明</span><textarea rows={2} value={input.description} onChange={event => change('description', event.target.value)} /></label><label><span>风险等级</span><select value={input.risk_summary} onChange={event => change('risk_summary', event.target.value)}><option value="read_only">只读</option><option value="local_action">本地操作</option><option value="network_write">网络写入</option><option value="approval_required">需要审批</option></select></label><label className="wide"><span>Capability 依赖</span><textarea rows={4} value={capabilityText} onChange={event => setCapabilityText(event.target.value)} placeholder="system.health|1.0.0|required|agent" /></label><label className="wide"><span>插件依赖</span><textarea rows={4} value={pluginText} onChange={event => setPluginText(event.target.value)} placeholder="com.himind.project-delivery|1.0.0|required" /></label><label className="wide"><span>SKILL.md</span><textarea className="skill-editor-readme" rows={15} value={input.readme} onChange={event => change('readme', event.target.value)} /></label>
  </div><div className="skill-dialog-actions"><button className="btn" onClick={onClose}>取消</button><button className="btn btn-primary" disabled={saving} onClick={save}>{saving ? '正在保存' : '保存草稿'}</button></div></div></div>;
}
